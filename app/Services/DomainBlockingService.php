...
class DomainBlockingService
{
    ...
    /**
     * Regenerate dnsmasq DHCP option 6 (DNS) for MACs that bypass the Pi resolver.
     *
     * Parent/guest/whitelisted devices receive upstream DNS so household
     * {@see BlockedWebsite} rules in dnsmasq do not apply to them. See
     * scripts/update_dnsmasq_dhcp_dns_bypass.sh.
     */
    public function syncDnsmasqDhcpDnsBypassForUser(User $user): bool
    {
        try {
            $primary = (string) config('services.dnsmasq.bypass_dns_primary', '8.8.8.8');
            $secondary = (string) config('services.dnsmasq.bypass_dns_secondary', '1.1.1.1');

            Log::info('Updating dnsmasq DHCP DNS bypass for user', [
                'user_id' => $user->id,
            ]);

            $macLines = Device::query()
                ->where('user_id', $user->id)
                ->get()
                ->filter(fn (Device $device) => ! $device->usesFilteredDns())
                ->pluck('mac_address')
                ->map(fn ($mac) => strtolower(trim((string) $mac)))
                ->unique()
                ->values()
                ->filter(fn ($mac) => (bool) preg_match('/^([0-9a-f]{2}:){5}[0-9a-f]{2}$/', (string) $mac));

            $stdin = $macLines->isEmpty() ? '' : $macLines->implode("\n")."\n";

            $result = $this->scriptExecutor->execute(
                'update_dnsmasq_dhcp_dns_bypass.sh',
                [$primary, $secondary],
                $stdin
            );

            if (! $result['success']) {
                Log::error('Failed to update dnsmasq DHCP DNS bypass', [
                    'user_id' => $user->id,
                    'error' => $result['error'] ?? 'Unknown error',
                    'output' => $result['output'] ?? '',
                ]);
            ...
            Log::error('Exception while updating dnsmasq DHCP DNS bypass', [
                'user_id' => $user->id,
                'error' => $e->getMessage(),
            ]);
