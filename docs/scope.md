...
- **DNS-based blocking via dnsmasq**: Redirect blocked domains to `127.0.0.1`
- **Configuration file**: `/etc/dnsmasq.d/blocked-domains.conf` (auto-generated, per-device)
- **Block types**:
  ...
  - **App-level**: Block app with all related domains (e.g., Facebook app blocks `facebook.com`, `api.facebook.com`, `graph.facebook.com`, `m.facebook.com`, etc. - 30+ domains for comprehensive coverage)
- **Related domain detection**: Auto-suggest related domains when blocking apps (e.g., blocking Facebook suggests all Facebook API domains, CDN domains, fallback domains)
- **dnsmasq integration**: Automatically update dnsmasq config and restart service when domains are added/removed (optimized to prevent service crashes)
...
- **Cached content limitation**: DNS blocking prevents new network requests but cannot remove content already cached on the device. Some cached videos/photos may still display, but app functionality (API calls, new content loading) is effectively blocked. This is expected behavior and acceptable - the app cannot function normally despite cached content.
...
11. **test-phase-6**: ✅ **COMPLETE** - Test full integration workflow (time expiration → portal → quiz/video → time grant) on local development environment. All automated tests (6 suites) and manual tests (7 tests) passed. Verified: time expiration detection, CheckTimeExpiration job, portal landing page, quiz flow, video flow, time granting, and complete end-to-end workflow. Portal Core system fully functional. See `docs/TEST_PHASE_6_RESULTS.md` for complete test results and procedures. Raspberry Pi testing recommended after TODOs #12 and #15 are complete.
12. **shell-scripts**: ✅ **COMPLETE** - Shell scripts created for network operations (block/unblock device, whitelist, get connected devices, monitor traffic) in scripts/ directory. All scripts tested and functional.
13. **test-phase-4**: ✅ **COMPLETE** - Test shell script execution and network control commands on Raspberry Pi. Verified PHP exec permissions, bash script execution, command output parsing, error handling, and security measures. All tests passed. NetworkService.getConnectedDevices() fixed. Sudoers configuration completed. See `docs/TEST_PHASE_4_RESULTS.md` for complete test results and `docs/TEST_PHASE_4_SETUP_REFERENCE.md` for setup reference.
14. **services**: ✅ **COMPLETE** - Service classes created and fully functional: TimeTrackingService ✅, TimeGrantingService ✅, NetworkService ✅, NoDogSplashService ✅, ScriptExecutor ✅. All services are operational and integrated with background jobs system.
15. **nodogsplash**: ✅ **COMPLETE** - NoDogSplash integration implemented: configuration management via bash scripts (redirect_device_portal.sh, allow_device_through.sh, check_device_redirected.sh), automatic redirect when time expires (intercept HTTP requests), redirect handling after portal completion (remove redirect), integration with time tracking. NoDogSplashService uses ScriptExecutor for secure script execution, following same pattern as NetworkService. Uses `ndsctl auth/deauth` commands to manage device authentication states. PortalController supports token-based MAC address lookup. Complete setup documented in `docs/NODOGSPLASH_SETUP.md`.
...
22. **websockets**: ✅ **COMPLETE** - Laravel Broadcasting with WebSockets implemented using Reverb + Echo. Added broadcasting/reverb configuration, private user channel authorization (`user.{id}`), and real-time event classes: `DeviceConnected`, `DeviceDisconnected`, `BlockedWebsiteAccessed`, `FlaggedWebsiteVisited`, `TimeExpired`, and `TimeGranted`. Event dispatch integrated into `MonitorDeviceConnections`, `CheckTimeExpiration`, `TimeGrantingService`, and `AccessAttempt` model creation hook. Dashboard frontend subscribes via Echo and renders live notifications in both the realtime grid card and welcome-row latest popup. Additional reliability fixes completed: removed duplicate script-stack rendering in layout (prevents duplicate subscriptions) and updated `MonitorDeviceConnections` to continue disconnection reconciliation even when connected-device scan is empty. End-to-end validation completed on local + Raspberry Pi: websocket handshake (`101`), `/broadcasting/auth` success (`200`), Echo state `connected`, all Step 22 event notifications verified, Reverb systemd service configured, and AP-side websocket access stabilized by allowing TCP 8080 in NoDogSplash `users-to-router` firewall rules. See `docs/websockets.md`, `docs/websockets_testing.md`, and `docs/websockets_raspberryPI.md` for implementation, test evidence, and deployment runbook.
23. **dashboard**: ✅ **COMPLETE** - Admin dashboard UI built with device overview, time status, recent activity, alerts, and real-time notification panel using Blade + Alpine.js. Dashboard redesigned using documentation patterns with yellow theme (#FFDE15), modern 3-column responsive grid layout (12-column system), Chart.js integration for graphical data representation, Feather Icons for SVG icons, Montserrat font, custom card designs with yellow borders and rounded corners, and responsive design ensuring all content fits within viewport without overflow. Navigation sidebar with auto-hide functionality, collapsible "General Settings" dropdown, and proper state management. Dashboard includes time usage cards, graphical representation charts, quiz results, and remaining video displays. All components fully responsive and styled according to DASHBOARD_AND_VIEWS_DOCUMENTATION.md patterns.
24. **time-tracking-bugfix-rpi-validation**: ✅ **COMPLETE** - Fixed and validated child device time deduction behavior on Raspberry Pi 4 production setup. Root causes addressed: (1) active sessions were not being created for connected active child devices (fixed by ensuring `TimeTrackingService::startSession()` is called from `MonitorDeviceConnections`), and (2) deduction was cumulative across job cycles instead of incremental (fixed by resetting session checkpoint after each deduction in `TimeTrackingService`). Additional clamp fix applied in `Device::deductTime()` to guarantee `remaining_time_minutes` never goes below 0 (`max(0, remaining - minutes)`). End-to-end RPi verification completed: git pull + service restart + scheduler/queue checks + live child device test confirmed internet access while time remains, periodic deduction during active use, and automatic cutoff when time is exhausted. See `docs/bugfix_timetracking.md` for procedures and validation notes.
